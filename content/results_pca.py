...
sample_cols = [
    ...
    if c not in ["ProteinName", "PeptideSequence", "log2FC", "p-adj", "stat", "p-value"]
]
unique_groups = sorted({group_map[s] for s in sample_cols if s in group_map})
...
expr_df = base_df.set_index("ProteinName")[sample_cols]
max_available = expr_df.shape[0]
